...
exports.rangeContentsString = (range) => {
    ...
    console.log('[Highlighter] Container HTML before processing:', container.innerHTML.substring(0, 500));
    console.log('[Highlighter] Script tags before cleanup:', container.querySelectorAll('script[type="math/mml"]').length);
    // MathJax 4 - extract script tags from inside mjx-containers BEFORE MathJax 2 cleanup
    // (mjx-containers have class="MathJax" so they'd be removed by the cleanup below)
    const mjxContainers = container.querySelectorAll('mjx-container');
    console.log('[Highlighter] Found', mjxContainers.length, 'mjx-container elements');
    mjxContainers.forEach((mjxContainer) => {
        const scriptTag = mjxContainer.querySelector('script[type="math/mml"]');
        console.log('[Highlighter] mjx-container has script tag:', !!scriptTag);
        if (scriptTag) {
            const content = scriptTag.textContent;
            console.log('[Highlighter] Script tag content:', content ? content.substring(0, 100) : 'null');
            if (mjxContainer.parentElement) {
                // Move the script tag to be a sibling of the mjx-container
                mjxContainer.parentElement.insertBefore(scriptTag, mjxContainer);
                console.log('[Highlighter] Moved script tag to be sibling');
            }
        }
    });
    // MathJax 2 cleanup (this also removes mjx-containers since they have class="MathJax")
    ...
    console.log('[Highlighter] Script tags after cleanup:', container.querySelectorAll('script[type="math/mml"]').length);
