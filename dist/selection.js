...
const normalizeStartWhitespace = (range) => {
    ...
    let walker = document.createTreeWalker(range.commonAncestorContainer, NodeFilter.SHOW_TEXT, null);
...
exports.snapSelection = (selection, options) => {
    ...
    const test0 = JSON.stringify(range.startContainer.textContent
    ...
    const test = JSON.stringify(range.startContainer.textContent
        ? range.startContainer.textContent.slice(range.startOffset, range.startOffset + 10)
        : '');
    console.log(test);
    console.log(range.startContainer.textContent);
